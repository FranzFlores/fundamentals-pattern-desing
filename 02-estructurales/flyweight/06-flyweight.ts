...
class LocationIcon implements Location {
    ...
    display(coordinates: { x: number; y: number; }): void {
      console.log(
        `Coordenadas: ${this.type} en ${coordinates.x}, ${coordinates.y} con ícono %c[${this.iconImage}]`,
        COLORS.green
      );
    }
}
...
const main = () => {
    ...
    const locations = [
        ...
        new MapLocation(36,66, factory.getLocationIcon('parque')),
        new MapLocation(37,67, factory.getLocationIcon('parque')),
        new MapLocation(38,68, factory.getLocationIcon('parque')),
    ];

    locations.map(location => location.display());
}
