...
class VendingMachine {
    ...
    setState(newState: State) {
        this.state = newState;
        console.log(`Estado cambió a: %c${newState.name}`);
    }
...
class ProductSelected implements State {
    ...
    insertMoney(): void {
        console.log('%c Por favor selecciona un producto - dinero insertado', COLORS.red);
    }
...
class DispensingProduct implements State {
    ...
    insertMoney(): void {
        console.log('%c Por favor espera que se entregue el producto', COLORS.red);
    }

    selectProduct(): void {
        console.log('%c Producto ya seleccionado y despachando', COLORS.red);
    }

    dispenseProduct(): void {
        console.log('%c Producto despachado. Cambiando estado a Esperando Dinero', COLORS.green);
    ...
    do {
        ...
        switch (selectedOption) {
            ...
            case '3':
                ...
                break;
            case '4':
                console.log(`%cSaliendo del sistema`, COLORS.blue);
                break;
            default:
